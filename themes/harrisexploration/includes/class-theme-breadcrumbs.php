...
class Theme_Breadcrumbs {
	...
	public function __construct(
		...
		$this->options   = wp_parse_args(
			$options,
			array(
				'separator'      => '<li class="breadcrumbs__separator">' . hny_get_svg( array( 'icon' => 'chevron-right' ) ) . '</li>',
		...
		$this->strings   = wp_parse_args(
			...
			array(
				...
				'search'    => array(
					'singular' => 'Search Results for <em>%s</em>',
					'plural'   => 'Search Results for <em>%s</em>',
				),
				'paged'     => 'Page %d',
				'404_error' => '404 Error',
			)
	...
	public function generate() {
		...
		if ( $this->options['posts_on_front'] ) {
			...
			if ( is_home() && ! $this->options['show_pagenum'] ) {
				$this->breadcrumb['blog'] = $this->template(
					get_the_title( $this->options['page_for_posts'] ),
					'current'
				);
			}
		}
		...
		if ( ! is_search() && ( is_singular() || ( is_archive() && ! is_post_type_archive() ) || $this->options['show_pagenum'] ) ) {
			$post_type_link = get_post_type_archive_link( $post_type );

			if ( $post_type_link ) {
				$post_type_label                            = 'post' === $post_type ? get_the_title( $this->options['page_for_posts'] ) : get_post_type_object( $post_type )->labels->name;
		...
		if ( is_singular() ) { // Posts, (Sub)Pages, Attachments and Custom Post Types
			if ( ! is_front_page() ) {
				...
				$this->breadcrumb[ "single_{$post_type}" ] = $this->template(
					get_the_title(),
					'current'
				);
			}
		} elseif ( is_search() ) { // Search
			$text                       = 'Search Results for <em>' . get_search_query() . '</em>';
			...
			if ( is_category() || is_tag() || is_tax() ) { // Categories, Tags and Custom Taxonomies
				...
				if ( 0 != $queried_object->parent && is_taxonomy_hierarchical(
					$taxonomy
				) ) { // Get Parents
				...
				$post_type_label                            = get_post_type_object(
					$post_type
				)->labels->name;
				$this->breadcrumb[ "archive_{$post_type}" ] = $this->template(
					$post_type_label,
				...
				$this->breadcrumb['archive_author'] = $this->template(
